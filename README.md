...
project/
│
├── 📂 [Data_Cleaning/](Data_Cleaning/)    # Python notebooks for data preparation
│   └── cleaning 1.ipynb                   # Data cleaning and preprocessing script
│
├── 📂 data/                               # [Download the Project Files from Google Drive](https://drive.google.com/drive/folders/1VoGovxHIo1BEpLPzocN6Y_r_JgMVFBIL?usp=sharing)
│   ├── project.pbix                       # Power BI Dashboard file
│   └── cleaned_*.csv                      # Cleaned CSV files used in the Power BI model
│
...
After downloading:
Simply extract the folder (if downloaded as a zip) and open `project.pbix` using Power BI Desktop. Since the dashboard and the CSV files are in the same folder, Power BI will successfully access the data without any path errors.
...
2️⃣ Download Files: Download the `data/` folder from the Downloads section.
3️⃣ Open Dashboard: Open `project.pbix` (located inside the downloaded folder) using Power BI Desktop.
